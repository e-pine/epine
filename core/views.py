...
# -----------------------------------Harvested Totals----------------------------------------
    harvested_good = BiddingProcess.objects.all().order_by('-date')
    overall_harvested_good_by_year = {}

    for bidding_process in harvested_good:
        year = bidding_process.date.year
        if year not in overall_harvested_good_by_year:
            overall_harvested_good_by_year[year] = []
        overall_harvested_good_by_year[year].append(bidding_process)
        bidding_process.total = bidding_process.calculate_total_harvest()

    # --HARVESTED BAD-- #
    harvested_bad = HarvestedBad.objects.all().order_by('date')
    overall_harvested_bad_by_year = {}

    for harvested_bads in harvested_bad:
        year = harvested_bads.date.year
        if year not in overall_harvested_bad_by_year:
            overall_harvested_bad_by_year[year] = []
        overall_harvested_bad_by_year[year].append(harvested_bads)
        harvested_bads.total = harvested_bads.calculate_total_harvest()

    # --REJECTED PINE-- #
    rejected_pines = RejectedPine.objects.all().order_by('date')
    overall_harvested_rejected_by_year = {}
    
    for rejected_pine in rejected_pines:
        year = rejected_pine.date.year
        if year not in overall_harvested_rejected_by_year:
            overall_harvested_rejected_by_year[year] = []
        overall_harvested_rejected_by_year[year].append(rejected_pine)
    ...
    # Sum the totals for each year
    overall_totals_by_year = {}

    for year in overall_harvested_good_by_year.keys() | overall_harvested_bad_by_year.keys():
        overall_harvested_good_total = sum(b.total for b in overall_harvested_good_by_year.get(year, []))
        overall_harvested_bad_total = sum(h.total for h in overall_harvested_bad_by_year.get(year, []))
        overall_harvested_rejected_total = sum(i.total for i in overall_harvested_rejected_by_year.get(year, []))

        overall_totals_by_year[year] = {
            'overall_harvested_good_total': overall_harvested_good_total,
            'overall_harvested_bad_total': overall_harvested_bad_total,
            'overall_harvested_rejected_total': overall_harvested_rejected_total,
            'overall_grand_total': overall_harvested_good_total + overall_harvested_bad_total + overall_harvested_rejected_total,
        }
# ___________________________________________________________________________________________________________________________
# -----------------PESONAL CHAT-----------------------
...
# -----------------PERSONAL CHAT------------------
        ...
        'overall_totals_by_year':overall_totals_by_year,
